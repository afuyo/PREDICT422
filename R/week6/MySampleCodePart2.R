...
#######################3
## Part A - Simple Linear Regression
modelA2 = lm(DAMT ~ LASTGIFT,data=regrData2,subset=trainIdx)
summary(modelA2)
par(mfrow=c(2,2))
plot(modelA2)
par(mfrow=c(1,1))
...
# try making this ch"ange to see if the error goes away. If the error remains, then
...
summary(modelB1)
###########################33
modelB1 = lm(DAMT ~ MAXRAMNT+LASTGIFT+MEDHVAL+MEDEDUC,data=regrData2,subset=trainIdx)
summary(modelB1)
